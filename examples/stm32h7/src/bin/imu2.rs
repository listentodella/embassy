...
lazy_static! {
    static ref IMU:Mutex<ThreadModeRawMutex,
     ICM42688<spi::ExclusiveDevice<stm32_spi::Spi<'static, embassy_stm32::mode::Blocking>, Output<'static>, spi::NoDelay>, icm426xx::Ready>> = {
        ...
        let icm = icm.initialize(Delay).unwrap();
    //     let a:Mutex<ThreadModeRawMutex, ICM42688<spi::ExclusiveDevice<stm32_spi::Spi<'_, embassy_stm32::mode::Blocking>, Output<'_>, spi::NoDelay>, icm426xx::Ready>> = Mutex::new(icm);
    //     a
        Mutex::new(icm)
    };
}
...
async fn watchdog_task() {
    loop {
        ...
        let mut ll = imu.ll();
        let mut bank = ll.bank::<{ icm426xx::register_bank::BANK0 }>();
        Timer::after_millis(1000).await;
    }
}
